const StrenWeak = () => {
  return (
    <div>
      <p className="text-xl font-bold mt-4 mb-2">Strengths 👍</p>
      <li className="lg:text-lg text-sm lg:mb-6 lg:mt-4 mb-2">
        <span className="font-bold text-accentOrange">Adaptability: </span> I adapt extremely
        quickly to new situations and can take charge if needed, even if the
        situation is tense or challenging.
      </li>
      <li className="lg:text-lg text-sm lg:mb-6 lg:mt-4 mb-2">
        <span className="font-bold text-accentOrange"> Jack of all trades:</span> I know quite a
        bit about a few topics, but a bit about pretty much everything.
      </li>
      <li className="lg:text-lg text-sm lg:mb-6 lg:mt-4 mb-2">
        <span className="font-bold text-accentOrange"> Peoples person:</span> I get along really
        well with other people, regardless if they are an old friend or I just
        met them 2 minutes ago.
      </li>

      <p className="text-xl font-bold mt-4 mb-2">Weaknesses 👎</p>
      <li className="lg:text-lg text-sm lg:mb-6 lg:mt-4 mb-2">
        <span className="font-bold text-accentOrange">Lack of planning:</span> I often skip
        planning and dive straight into projects, which can lead to headaches
        when things don't go as smoothly as expected.
      </li>
      <li className="lg:text-lg text-sm lg:mb-6 lg:mt-4 mb-2">
        <span className="font-bold text-accentOrange">Overextending myself:</span> I get excited
        by lots of things and sometimes (okay, almost all the time) that means I
        spread myself too thin.
      </li>
      <li className="lg:text-lg text-sm lg:mb-6 lg:mt-4 mb-2">
        <span className="font-bold text-accentOrange">Impostor syndrome:</span> Like many
        programmers, I sometimes doubt my skills, making me hesitant to suggest
        improvements for example.
      </li>
    </div>
  );
};

export default StrenWeak;
